# Macsym — loja virtual e painel administrativo

Loja de câmeras PTZ, microfones, mesas controladoras e acessórios de áudio e
vídeo para videoconferência, com painel administrativo próprio.
...
# 2. Banco de dados
mysql -u root -e "CREATE DATABASE macsym CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
...
# 4. Catálogo: importe do site atual, gere a semente, crie as tabelas
npm run importar:woo    # camerasdevideo.com.br -> src/data.ts (71 produtos)
...
`public/banners/`). Enquanto as fotos não forem baixadas, a vitrine aponta para
o WordPress atual — e o CSP de produção bloqueia domínio de terceiro, então a
página sai sem imagem. Para baixar os originais:
...
| `npm run importar:woo` | Lê o WooCommerce atual e reescreve `src/data.ts` (produtos, seções e subcategorias) |
...
curl -H "Authorization: Bearer qp_live_..." \
     https://camerasdevideo.com.br/api/v1/products
```
