...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL ?? 'admin@camerasdevideo.com.br';
const ADMIN_PASS = process.env.ADMIN_PASS ?? 'DemoMacsym2026!';
const log = [];
...
title.includes('Macsym') ? ok('título da página') : fail('título: ' + title);
...
if (await addBtn.count()) {
  ...
  const stored = await page.evaluate(() => localStorage.getItem('macsym_cart_v1'));
  stored && JSON.parse(stored).length === 1 ? ok('item salvo no localStorage') : fail('carrinho não persistiu: ' + stored);
  await page.reload({ waitUntil: 'networkidle' });
  const after = await page.evaluate(() => localStorage.getItem('macsym_cart_v1'));
...
adminBody.includes(ADMIN_EMAIL) ? ok('usuário exibido no topo') : fail('usuário não exibido');
