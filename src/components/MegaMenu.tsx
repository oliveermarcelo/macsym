        ...
        {open && (
          ...
          >
            <div className="bg-white rounded-2xl shadow-[0_24px_60px_rgba(43,49,37,0.18)] border border-gray-100 overflow-hidden flex w-[840px] max-w-[92vw]">
              ...
              <div className="w-[280px] bg-gradient-to-b from-gray-50 to-white border-r border-gray-100 py-3 max-h-[460px] overflow-y-auto">
                {menu.map((cat) => {
                  ...
                  return (
                    ...
                    >
                      {isActive && <span className="absolute left-0 top-1.5 bottom-1.5 w-1 rounded-r-full bg-brand-accent-300" />}
                  ...
                  <ul className="grid grid-cols-2 gap-x-5 gap-y-1">
                    {activeCategory.subcategories.map((sub) => (
                      <li key={sub.id}>
                        ...
                        >
                          <span className="w-1.5 h-1.5 rounded-full bg-gray-200 group-hover:bg-brand-accent-300 transition-colors flex-shrink-0" />
                          {sub.name}
                        </button>
                      </li>
                    ))}
              ...
              {/* Promo column */}
              <div className="hidden lg:flex w-[220px] flex-shrink-0 bg-gradient-to-br from-[#0f1318] via-primary-blue to-primary-container text-white p-6 flex-col justify-between relative overflow-hidden">
                <Sparkles size={90} className="absolute -right-3 -bottom-3 text-white/10" />
                <div className="relative">
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-accent-300">Destaques</span>
                  <p className="mt-2 text-lg font-extrabold leading-tight">Os equipamentos que mais saem</p>
                  <p className="text-xs text-white/60 mt-2">Câmeras PTZ, microfones e controladoras selecionados.</p>
                </div>
                <button
                  onClick={() => handleSelect('destaques')}
                  className="relative mt-4 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-full bg-brand-accent-300 text-primary-blue text-xs font-bold uppercase tracking-wider hover:bg-white transition-colors"
                >
