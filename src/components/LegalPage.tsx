  ...
  return partes.map((parte, i) =>
    /^[\w.+-]+@[\w-]+\.[\w.-]+$/.test(parte) ? (
      <a
        key={i}
        href={`mailto:${parte}`}
        className="text-primary-blue font-medium underline underline-offset-2 hover:text-brand-accent-300 transition-colors"
      >
        {parte}
      </a>
    ) : (
            ...
            <section key={i} className="space-y-3">
              ...
              {secao.blocks.map((bloco, j) =>
                bloco.items.length > 0 ? (
                  <ul
                    key={j}
                    className="list-disc pl-5 space-y-2 text-[15px] text-gray-700 leading-relaxed marker:text-brand-accent-300"
                  >
