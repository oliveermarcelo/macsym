...
export default function Footer({ onOpenStory, onOpenCertifications, onOpenLegal }: FooterProps) {
  ...
  return (
    <footer id="main-footer" className="relative bg-[#0f1318] text-white overflow-hidden">
      <div className="absolute -top-24 right-1/4 w-96 h-96 rounded-full bg-brand-accent-300/5 blur-3xl pointer-events-none" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col lg:flex-row items-center justify-between gap-6">
          <div className="text-center lg:text-left">
            <h3 className="text-xl sm:text-2xl font-extrabold">
              Receba novidades e ofertas <span className="text-brand-accent-300">exclusivas</span>
            </h3>
          ...
          <form onSubmit={handleSubscribe} className="w-full lg:w-auto">
            <div className="flex gap-2 w-full lg:w-[420px]">
              <input
                ...
                className="flex-1 text-sm rounded-xl px-4 py-3 bg-white/10 border border-white/15 text-white placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-brand-accent-300/40 focus:border-brand-accent-300/50"
              />
              <button
                type="submit"
                className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-brand-accent-400 hover:bg-brand-accent-300 text-brand-ink font-bold text-sm uppercase tracking-wider transition-colors whitespace-nowrap"
              >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-10">
          ...
          <div className="col-span-2 lg:col-span-4 space-y-5">
            ...
            <p className="text-sm text-white/60 leading-relaxed max-w-xs">
              ...
              auditórios e salas de aula. Equipamento profissional, com suporte técnico
            ...
            <div className="flex items-center gap-2.5">
              {[Instagram, Facebook].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  className="w-9 h-9 rounded-full bg-white/10 hover:bg-brand-accent-300 hover:text-primary-blue flex items-center justify-center text-white/80 transition-colors"
                >
                  <Icon size={17} />
                </a>
              ))}
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="space-y-2.5 text-sm text-white/60">
              {['Câmeras PTZ', 'Microfones', 'Mesas Controladoras', 'Webcams', 'Acessórios'].map((c) => (
                <li key={c}><a href="#" className="hover:text-brand-accent-300 transition-colors">{c}</a></li>
              ))}
            </ul>
          </div>
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="space-y-2.5 text-sm text-white/60">
              <li><button onClick={onOpenStory} className="hover:text-brand-accent-300 transition-colors">Sobre nós</button></li>
              <li><button onClick={onOpenCertifications} className="hover:text-brand-accent-300 transition-colors">Qualidade & Garantia</button></li>
              {/*
              ...
              {LEGAL_DOCS.map((doc) => (
                <li key={doc.slug}>
                  <a
                    ...
                    className="hover:text-brand-accent-300 transition-colors"
                  >
                    {doc.navLabel}
                  </a>
                </li>
