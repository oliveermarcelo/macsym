...
export default function IntegrationsAdmin() {
  ...
  return (
    <div className="space-y-8">
      {/* Hero header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0f1318] via-primary-blue to-primary-container text-white p-6 sm:p-8">
        <Plug size={150} className="absolute -right-6 -bottom-8 text-white/10" />
        <div className="absolute -top-10 -left-10 w-40 h-40 rounded-full bg-brand-accent-300/15 blur-3xl" />
...
function IntegrationCard({ provider }: { provider: ProviderMeta; key?: React.Key }) {
  ...
  return (
    <div className={`group bg-white rounded-2xl border shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden ${
      provider.native ? 'border-brand-accent-300/40 ring-1 ring-brand-accent-300/20' : 'border-gray-150'
    }`}>
      {/* Top accent strip */}
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {provider.native && (
                  <span className="inline-flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-wider bg-brand-accent-300/15 text-brand-accent-300 px-2 py-0.5 rounded-full">
