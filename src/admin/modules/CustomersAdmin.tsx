...
export default function CustomersAdmin() {
  ...
  const openWhatsApp = (phone: string, name: string) => {
    const text = encodeURIComponent(`Olá, ${name.split(' ')[0]}! Aqui é da Macsym. 👋`);
...
function EmailModal({ customer, onClose }: { customer: Customer; onClose: () => void }) {
  const [subject, setSubject] = useState('Macsym — novidades pra você');
  const [body, setBody] = useState(
    `Olá, ${customer.name.split(' ')[0]}!\n\nTemos novidades e ofertas especiais selecionadas pra você na Macsym. Aproveite!\n\nEquipe Macsym`
  );
