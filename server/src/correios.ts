...
export async function cotar(
  ...
  if (!preco.ok) {
     ...
     */
    console.error(
      `[macsym] Correios recusaram a cotação de ${servico} (HTTP ${preco.status}):`,
  ...
  try {
    ...
    if (prazo.ok) {
      ...
      if (dias === 0) {
        console.warn(
          `[macsym] Correios não devolveram prazo para ${servico}:`,
          String(prazo.body ?? '').slice(0, 300),
        );
      }
    } else {
      console.warn(
        `[macsym] falha ao consultar o prazo de ${servico} (HTTP ${prazo.status}):`,
        String(prazo.body ?? '').slice(0, 300),
      );
    }
