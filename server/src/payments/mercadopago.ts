...
function chaveIdempotencia(orderId: string, tentativa: number): string {
  return `macsym-${orderId}-${tentativa}`;
}
...
export async function cobrar(
  ...
  try {
    ...
    console.error('[macsym] falha ao cobrar no Mercado Pago:', err.status ?? '', err.message ?? e);
    fail(
...
export async function consultarPedido(
  ...
  try {
    ...
    console.error('[macsym] falha ao consultar pedido no Mercado Pago:', ref, err.status ?? '', err.message ?? e);
    return null;
  }
}
