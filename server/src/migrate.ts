  ...
  for (const p of catalog.products ?? []) {
    await q.run(
      ...
      [
         ...
         *
         * `weight` aceita os dois formatos porque o catálogo teve os dois: no
         * formato atual (o que `src/types.ts` declara) ele é o peso em quilos,
         * já numérico, e o rótulo da vitrine vive em `weightLabel`; no formato
         * antigo era texto livre ("1,2 kg") fazendo os dois papéis. Chamar
         * `.match()` num número derrubava a importação inteira na primeira
         * linha do catálogo novo.
         */
...
async function seedDemoOrders(): Promise<void> {
  ...
  const pool: [string, string][] = [
    ...
    ['Integra AV Projetos', 'compras@integraav.com.br'],
    ['Carla Mendes', 'carla.mendes@email.com'],
    ['Colégio Monte Azul', 'compras@monteazul.edu.br'],
    ['Lucas Ferreira', 'lucas.ferreira@email.com'],
    ['Juliana Prado', 'juliana.prado@email.com'],
    ['Studio Onda Live', 'producao@ondalive.com.br'],
  ];
  ...
  await transaction(async (tx) => {
    ...
    const samples: [string, string, string, number][] = [
      ...
      ['Integra AV Projetos', '5511966665555', 'compras@integraav.com.br', 26],
...
main()
  .catch((e) => {
    console.error('[macsym] falha na migração:', e);
    process.exitCode = 1;
  })
  .finally(() => {
    void closePool();
