...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error('e o nome do banco/usuário leva o prefixo da conta (ex.: u123456789_macsym).');
    process.exit(1);
  }
  ...
  if (process.env.AUTO_MIGRAR !== 'false') {
    try {
      ...
      const { tabelas, colunas, indices, convertidas } = await sincronizarEstrutura(
        (m) => console.log('[macsym]' + m),
      );
      if (tabelas > 0 || colunas > 0 || indices > 0 || convertidas > 0) {
        console.log(
          `[macsym] banco atualizado na subida: ${tabelas} tabela(s), ${colunas} coluna(s), `
      ...
      console.error(
        '[macsym] não consegui conferir a estrutura do banco:',
        err.code ?? '',
        err.message ?? e,
      );
      console.error('[macsym] rode `node migrate.js` se o checkout reclamar de banco desatualizado.');
    }
  }

  const app = createApp();
  const server = app.listen(config.port, config.host, () => {
    console.log(`[macsym] no ar em http://${config.host}:${config.port} (${config.env})`);
  ...
  const shutdown = (sinal: string) => {
    console.log(`[macsym] recebi ${sinal}, encerrando…`);
    server.close(() => {
...
main().catch((e) => {
  console.error('[macsym] falha ao iniciar:', e);
  process.exit(1);
});
