...
publicRoutes.post('/orders', h(async (req, res) => {
  ...
  if (ufDoCep !== '' && ufDigitada !== '' && ufDigitada !== ufDoCep) {
    console.warn(
      `[macsym] UF corrigida pelo CEP no pedido: ${cep} é ${ufDoCep}, veio ${ufDigitada}`,
    );
  }
  if (
  ...
  try {
    ...
    console.error('[macsym] falha ao gravar pedido:', e);
  ...
  const dadosBase = {
    orderId,
    total: quote.total,
    descricao: `Pedido ${orderId} — Macsym`,
    pagador: {
      email,
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[macsym] pedido ${orderId}: a cobrança falhou E o cancelamento também.`,
  ...
  if (ambiente(cred) === 'teste') {
    console.log(`[macsym] pedido ${orderId} cobrado em AMBIENTE DE TESTE — nenhum dinheiro se moveu.`);
  }
