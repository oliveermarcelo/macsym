...
webhookRoutes.post('/mercadopago', h(async (req, res) => {
  ...
  if (cred === null) {
    console.error('[macsym] webhook do Mercado Pago recebido sem credenciais cadastradas');
  ...
  if (assinatura.ok === false) {
     ...
     */
    console.error(
      '[macsym] webhook do Mercado Pago com assinatura inválida:',
      assinatura.motivo,
  ...
  if (orderId === '') {
    console.error('[macsym] webhook do Mercado Pago sem pedido correspondente:', dataId, tipo);
  ...
  console.log(
    `[macsym] webhook ${PROVEDOR}: pedido ${orderId} → ${real.status} (${real.detalhe})`,
