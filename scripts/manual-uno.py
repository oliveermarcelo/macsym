#!/usr/bin/env python3
"""
Gera o manual de integração Macsym ↔ UNO ERP (PDF).
...
VERDE = colors.HexColor('#262c34')
VERDE_ESCURO = colors.HexColor('#1a1f25')
COBRE = colors.HexColor('#c2202c')
...
BASE = 'https://camerasdevideo.com.br'
...
def rodape(canvas, doc):
    ...
    canvas.drawString(22 * mm, 12 * mm, 'Macsym ↔ UNO ERP — manual de integração')
...
def capa(canvas, doc):
    ...
    canvas.drawString(22 * mm, A4[1] - 104 * mm, 'Macsym  ↔  UNO ERP')
    canvas.setStrokeColor(colors.HexColor('#c2202c'))
...
def construir(saida='Manual-Integracao-Macsym-UNO-ERP.pdf'):
    doc = BaseDocTemplate(
        saida, pagesize=A4,
        leftMargin=22 * mm, rightMargin=22 * mm, topMargin=20 * mm, bottomMargin=20 * mm,
        title='Manual de integração — Macsym ↔ UNO ERP',
        author='Macsym', subject='Integração de e-commerce com ERP',
    )
    ...
    add(bloco("""
UNO ERP                                     Loja (camerasdevideo.com.br)
   |                                             |
...
Loja                                        UNO ERP
   ...
   |  X-Macsym-Event: order.created              |
   |-------------------------------------------->|   pedido novo, já pago
   |                                             |
   |  X-Macsym-Event: order.status_changed       |
   |-------------------------------------------->|   mudança de status
"""))
    add(Paragraph(
...
"""))
    ...
    add(bloco("""
curl -s https://camerasdevideo.com.br/api/v1/products \\
...
"""))
    ...
    add(Paragraph(
        ...
        'POST com JSON e o header <font face="Courier">X-Macsym-Event</font>.', P))
...
"""))
    ...
    add(bloco("""
X-Macsym-Event: order.created
X-Macsym-Signature: t=1787061681,v1=<hmac_sha256(segredo, "t.corpo")>
"""))
