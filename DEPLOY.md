...
| `DB_NAME` | o nome mostrado na lista — leva o prefixo da conta (`u123456789_macsym`) |
...
| Application root | `macsym` (a pasta onde você vai subir os arquivos) |
...
Entre na pasta `macsym` e envie **todo o conteúdo** de `deploy/` (não a pasta
`deploy` em si).
...
| `APP_URL` | `https://camerasdevideo.com.br` |
| `APP_ENV` | `production` |
| `SECURE_COOKIES` | `true` |
...
```bash
cd ~/macsym                 # a pasta da aplicação
node migrate.js --admin-email=voce@camerasdevideo.com.br --admin-pass='SenhaForte123'
```
...
```bash
~/nodevenv/macsym/22/bin/node migrate.js --admin-email=... --admin-pass='...'
```
...
|---|---|
| **"We're sorry, but something went wrong"** | erro na inicialização. Veja `~/macsym/stderr.log` (ou o botão de log na tela do Node) |
